...
import {
  createShareCard,
  getShareCardBySlug,
  readShareBeforeImageBuffer,
  readShareImageBuffer,
  shareBeforeImageExists,
  type SharePathType,
...
shareRouter.get('/share/cards/:slug', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const hasBeforeImage = await shareBeforeImageExists(card.slug);
      return res.json({
        ok: true,
        slug: card.slug,
...
shareRouter.get('/share/cards/:slug/before', async (req, res) => {
  const { slug } = req.params;
  if (!slug) {
    return res.status(400).json({ ok: false, error: 'slug is required' });
  }
  ...
  try {
    ...
    try {
      ...
      const file = await readShareBeforeImageBuffer(card.slug);
      if (!file) {
        return res.status(404).json({ ok: false, error: 'not_found' });
      }
      res.setHeader('Content-Type', file.contentType);
    ...
    console.error('share/cards before error', error);
    return res.status(500).json({ ok: false, error: 'internal_error' });
  }
});
