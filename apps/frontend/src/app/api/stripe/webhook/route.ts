...
export async function POST(request: NextRequest) {
  ...
  if (isGcpPrimaryEnabled()) {
    try {
      ...
      console.error('[Webhook][GCP] Error processing webhook event:', error);
      return NextResponse.json(
        { error: 'Error processing GCP webhook event', details: error.message },
        { status: 500 }
      );
    }
  }
...
async function handleWebhookEvent(event: Stripe.Event) {
  if (isGcpPrimaryEnabled()) {
    await handleWebhookEventWithGcp(event);
    return;
  }
...
async function handleWebhookEventWithGcp(event: Stripe.Event) {
  switch (event.type) {
    case 'checkout.session.completed': {
      const session = event.data.object as Stripe.Checkout.Session;
      ...
      if (!subscriptionId || !userHash) {
        throw new Error('Missing subscriptionId or userHash for checkout.session.completed');
      }
    ...
    case 'customer.subscription.updated': {
      ...
      if (!result.ok) throw new Error(result.error || 'GCP subscription update failed');
      return;
    }
    ...
    case 'customer.subscription.deleted': {
      ...
      if (!result.ok) throw new Error(result.error || 'GCP subscription delete failed');
      return;
    }
    ...
    case 'invoice.payment_succeeded': {
      ...
      const subscription = await stripe.subscriptions.retrieve(subscriptionId);
      const planId = subscription.metadata?.plan_id as 'basic' | 'pro' | 'studio';
      const billingPeriod = subscription.metadata?.billing_period as 'monthly' | 'yearly';
      const planConfig = getPlanConfig(planId, billingPeriod);
      ...
      if (!result.ok) throw new Error(result.error || 'GCP invoice success sync failed');
      return;
    }
    ...
    case 'invoice.payment_failed': {
      ...
      if (!result.ok) throw new Error(result.error || 'GCP invoice failure sync failed');
      return;
    }

    default:
      console.log(`[Webhook][GCP] Unhandled event type: ${event.type}`);
  }
}
