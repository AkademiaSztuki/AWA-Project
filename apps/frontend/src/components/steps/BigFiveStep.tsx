...
export function BigFiveStep({ data, onUpdate, onNext, onBack }: BigFiveStepProps) {
  const { language } = useLanguage();
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [responses, setResponses] = useState<Record<string, number>>(data?.responses || {});
  const [showResults, setShowResults] = useState(false);
  const [scores, setScores] = useState<IPIPScores | null>(data?.scores || null);
  ...
  useEffect(() => {
    if (data) {
      ...
      if (Object.keys(data.responses).length === IPIP_60_ITEMS.length) {
        setShowResults(true);
      }
    }
  }, [data]);
  ...
  const handleResponse = (value: number) => {
    const safeCurrentQuestion = Math.min(currentQuestion, IPIP_60_ITEMS.length - 1);
    const item = IPIP_60_ITEMS[safeCurrentQuestion];
    if (!item) return;
    ...
    setTimeout(() => {
      if (safeCurrentQuestion < IPIP_60_ITEMS.length - 1) {
        setCurrentQuestion(prev => Math.min(prev + 1, IPIP_60_ITEMS.length - 1));
      } else {
        // Last question - calculate scores
        const finalScores = calculateIPIPScores(newResponses);
  ...
  const safeCurrentQuestion = Math.min(currentQuestion, IPIP_60_ITEMS.length - 1);
  const progress = ((safeCurrentQuestion + 1) / IPIP_60_ITEMS.length) * 100;
  const currentItem = IPIP_60_ITEMS[safeCurrentQuestion];
    ...
    <GlassCard className="p-6 md:p-8 min-h-[600px] max-h-[85vh] overflow-auto scrollbar-hide">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-modern text-graphite">
            {t("Pytanie", "Question")} {safeCurrentQuestion + 1} / {IPIP_60_ITEMS.length}
          </span>
      ...
      <div className="mb-8">
        <div className="text-center mb-6">
          <h3 className="text-xl lg:text-2xl font-nasalization text-graphite mb-4">
            {currentItem?.text[language] || t("Błąd ładowania pytania", "Error loading question")}
          </h3>
          <p className="text-sm text-silver-dark font-modern">
          ...
          {[1, 2, 3, 4, 5].map((value) => (
            <motion.button
              ...
              className={`p-4 rounded-xl font-modern transition-all duration-300 ${
                currentItem && responses[currentItem.id] === value
                  ? 'bg-gradient-to-br from-gold to-champagne text-white shadow-lg'
                  : 'glass-panel hover:bg-white/20 text-graphite'
              }`}
            >
