...
export const useAudioManager = (): AudioManager => {
  ...
  const stopAllDialogueAudio = useCallback(() => {
    // #region agent log
    void fetch('http://127.0.0.1:7242/ingest/03aa0d24-0050-48c3-a4eb-4c5924b7ecb7',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sessionId:'debug-session',runId:'audio-debug',hypothesisId:'A8',location:'useAudioManager.ts:stopAllDialogueAudio',message:'stopAllDialogueAudio invoked',data:{registeredCount:dialogueAudios.current.size,playingCount:Array.from(dialogueAudios.current).filter(a=>a && !a.paused).length,registeredAudios:Array.from(dialogueAudios.current).map(a=>({src:a.src,paused:a.paused,currentTime:a.currentTime,duration:a.duration}))},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
    console.log('Stopping all dialogue audio...');
    dialogueAudios.current.forEach(audio => {
      if (audio && !audio.paused) {
        // #region agent log
        void fetch('http://127.0.0.1:7242/ingest/03aa0d24-0050-48c3-a4eb-4c5924b7ecb7',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sessionId:'debug-session',runId:'audio-debug',hypothesisId:'A9',location:'useAudioManager.ts:stopAllDialogueAudio-pause',message:'Pausing audio via stopAllDialogueAudio',data:{src:audio.src,currentTime:audio.currentTime,duration:audio.duration,remaining:audio.duration>0?audio.duration-audio.currentTime:0},timestamp:Date.now()})}).catch(()=>{});
        // #endregion
