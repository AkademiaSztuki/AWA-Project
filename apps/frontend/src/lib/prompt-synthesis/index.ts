...
export async function synthesizeSixPrompts(
  ...
  const synthesisPromises = availableSources.map(async (source) => {
    ...
    // Special handling for InspirationReference - use concrete VLM tags
    let result: SynthesisResult;
    if (source === GenerationSource.InspirationReference) {
      // Extract concrete tags from inspiration images using the same function as scoring
      // We need to replicate the logic since integrateInspirationTags is not exported
      const extractInspirationTags = (inspirations: PromptInputs['inspirations']) => {
        if (!inspirations || inspirations.length === 0) {
          return { additionalStyles: [], additionalColors: [], additionalMaterials: [], biophiliaBoost: 0 };
        }
        ...
        inspirations.forEach(inspiration => {
          if (inspiration.tags) {
            inspiration.tags.styles?.forEach(style => {
              // Extract valid style (simple check for now)
              const validStyle = style.toLowerCase().trim();
              if (['modern', 'scandinavian', 'industrial', 'minimalist', 'rustic', 'bohemian', 
                   'contemporary', 'traditional', 'mid-century', 'japandi', 'coastal', 'farmhouse',
                   'mediterranean', 'art-deco', 'maximalist', 'eclectic', 'hygge', 'zen', 'vintage', 'transitional'].includes(validStyle)) {
      ...
      const simpleWeights = calculatePromptWeights(filteredInputs);
      
      // Build prompt with concrete tags instead of generic description
      ...
      // Add concrete tags from VLM analysis
      ...
      if (inspirationTags.additionalColors.length > 0) {
        promptJson.reference_colors = inspirationTags.additionalColors.slice(0, 4);
      ...
      if (inspirationTags.additionalMaterials.length > 0) {
        promptJson.reference_materials = inspirationTags.additionalMaterials.slice(0, 3);
      ...
      if (!promptJson.reference_styles && !promptJson.reference_colors && !promptJson.reference_materials) {
      ...
      const explainability = generateExplainabilityMetadata(simpleWeights, filteredInputs, source);
      
      result = {
        prompt: JSON.stringify(promptJson, null, 2),
        components: {
          roomType: roomType,
          style: simpleWeights.dominantStyle || '',
          mood: '',
          colors: '',
          materials: '',
          lighting: '',
