...
import { buildNanoBananaJsonPrompt, validatePromptLength, PromptComponents, ensureHexColor, getBiophiliaDescriptors, getComplexityLabel, buildStructuredPromptObject, PROMPT_SCHEMA_VERSION } from './builder';
import { 
  GenerationSource, 
...
import { generateLayoutVariation } from './layout-diversity';
...
export async function synthesizePrompt(
  ...
  let finalPrompt = buildNanoBananaJsonPrompt(weights, roomType, options.sourceType);
...
export async function synthesizeSixPrompts(
  ...
  const synthesisPromises = availableSources.map(async (source) => {
    ...
    // Special handling for InspirationReference - use concrete VLM tags from Gemini 2.5 Flash-Lite
    ...
    if (source === GenerationSource.InspirationReference) {
      ...
      // Tags are generated by Gemini 2.5 Flash-Lite and stored in Supabase
      ...
      const extractInspirationTags = (inspirations: PromptInputs['inspirations']) => {
        ...
        return {
          additionalStyles: getTop(styleCounts, 5),
          additionalColors: getTop(colorCounts, 8),
          additionalMaterials: getTop(materialCounts, 6),
          biophiliaBoost,
      ...
      const inspirationOnlyWeights: PromptWeights = {
        ...
        dominantStyle: inspirationTags.additionalStyles.length > 0 
          ...
          : '',
        secondaryStyles: inspirationTags.additionalStyles.slice(1, 3),
      ...
      // Build JSON prompt (new narrative structure) with concrete tags from inspirations ONLY
      const roomName = roomType === 'living room' ? 'living room' : 
                      roomType === 'bedroom' ? 'bedroom' :
                      roomType === 'kitchen' ? 'kitchen' : roomType;
      
      // MINIMAL JSON for InspirationReference - same structure as other sources
      const primaryStyle = inspirationTags.additionalStyles.length > 0
        ? inspirationTags.additionalStyles[0]
        : 'modern';
      ...
      // Approximate complexity/brightness for InspirationReference (neutral defaults)
      const complexity = inspirationOnlyWeights.visualComplexity < 0.35 ? 'simple'
        : inspirationOnlyWeights.visualComplexity > 0.70 ? 'complex'
        : 'balanced';

      const brightness = 'balanced';
      const lightingMood = 'neutral';
      const natureMetaphor = filteredInputs.sensory?.natureMetaphor || '';
      const mood = 'from reference images';

      const plants = (() => {
        const d = Math.max(0, Math.min(1, blendedNatureDensity || 0));
        if (d <= 0.05) return 0;
        if (d <= 0.17) return 1;
        if (d <= 0.34) return 2;
        if (d <= 0.50) return 4;
      ...
      // Source-specific extra fields: raw VLM labels — critical when whitelist/hex mapping drops structured tags
      const inspirationExtra: Record<string, unknown> = {};
      if (visionSummary) inspirationExtra.vision_summary = visionSummary;
      if (visionStyleLabels.length > 0) inspirationExtra.vision_style_labels = visionStyleLabels;
      if (visionColorLabels.length > 0) inspirationExtra.vision_color_labels = visionColorLabels;
      const inspLayout = generateLayoutVariation(GenerationSource.InspirationReference, inspirationOnlyWeights);
      if (inspLayout?.description) inspirationExtra.layout = inspLayout.description;

      // Shared schema: identical core keys as the other 5 sources + architectural lock + schema_version
      ...
      console.log('[InspirationReference] Built simplified JSON prompt:', {
        ...
        secondary_styles: inspirationTags.additionalStyles.slice(1, 3),
        colorsCount: hexColorsDeduped.length,
        materialsCount: allMaterials.length,
        plants,
        hasVisionSummary: !!visionSummary,
...
export {
  ...
  buildNanoBananaJsonPrompt,
  buildStructuredPromptObject,
