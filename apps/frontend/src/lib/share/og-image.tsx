...
import { shareOgCopy, type ShareLanguage } from '@/lib/share/captions';
...
function photoFrame(src: string | null, label: string) {
  return (
    <div
      style={{
        display: 'flex',
...
function ogLabels(language: ShareLanguage): {
  before: string;
  after: string;
  footer: string;
} {
  if (language === 'en') {
    return {
      before: 'Before',
      after: 'After',
      footer: 'Generate yours · project-ida.com',
    };
  }
  return {
    before: 'Przed',
    after: 'Po',
    footer: 'Wygeneruj swoją koncepcję · project-ida.com',
  };
}
...
export async function buildShareOgImageResponse(
  ...
  const [afterSrc, beforeSrc] = trimmed
    ? await Promise.all([fetchShareImage('image', trimmed), fetchShareImage('before', trimmed)])
    : [null, null];

  const ready = Boolean(beforeSrc && afterSrc);
  const labels = ogLabels(language);
  ...
  const imageResponse = new ImageResponse(
    (
      ...
      >
        ...
        >
          {photoFrame(beforeSrc, labels.before)}
          {photoFrame(afterSrc, labels.after)}
        </div>
