# Upload 17_blind_selection_and_dominant_tags.sql to GCS and import into Cloud SQL.
# Usage (from infra/gcp): .\run-blind-dominant-tags-migration.ps1
...
$SqlFile = Join-Path $InfraDir "sql\17_blind_selection_and_dominant_tags.sql"
if (-not (Test-Path $SqlFile)) {
    Write-Host "Missing file: sql\17_blind_selection_and_dominant_tags.sql" -ForegroundColor Red
    exit 1
}

$GcsUri = "gs://$BUCKET/setup/17_blind_selection_and_dominant_tags.sql"
...
Write-Host "Done. blind_selection_made + implicit_dominant_tags columns applied in $Database." -ForegroundColor Green
