# Upload 21_promo_codes.sql to GCS and import into Cloud SQL.
# Usage (from infra/gcp): .\run-promo-codes-migration.ps1
...
$SqlFile = Join-Path $InfraDir "sql\21_promo_codes.sql"
if (-not (Test-Path $SqlFile)) {
    Write-Host "Missing file: sql\21_promo_codes.sql" -ForegroundColor Red
    exit 1
}

$GcsUri = "gs://$BUCKET/setup/21_promo_codes.sql"
...
Write-Host "Done. promo_codes tables applied in $Database." -ForegroundColor Green
